...
def show_tool(ID):
    ...
    for column_value in tools.view_tool(ID):
        ...
        print(f": Categorias : {column_value[4]}")
...
def input_data_register():
    ...
    categories = input(f"{cmd}-[Categorias separadas por espaços]:~> ")
...
def find_item_index(item_arg):
    list_items = tools.table_info()
    list_items_index = []
